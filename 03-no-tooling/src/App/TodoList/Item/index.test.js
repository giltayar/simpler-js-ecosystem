...
const App = () => {
  const [appState] = useRecoilState(recoilState);
  if (appState.todoList.length === 0) return null;
  return _jsx("div", { children: _jsx(Item, { todo: appState.todoList[0] }) });
};
test("should each initialAppstate todo object value is set to Item element", () => {
  TestRenderer(
    _jsx(Item, { todo: initialRecoilState.todoList[0] }),
    initialRecoilState
  );
  expect(screen.getByTestId("todo-item")).toBeInTheDocument();
  expect(screen.getByTestId("todo-item-complete-check").checked).toBe(false);
  expect(screen.getByTestId("todo-body-text")).toHaveTextContent("cut tomato");
  expect(screen.getByTestId("todo-edit-input").value).toBe("cut tomato");
});
test("should set css classes correctly", () => {
  TestRenderer(_jsx(App, {}), initialRecoilState);
...
test("should work todo completed checkbox", () => {
  ...
  expect(screen.getByTestId("todo-item-complete-check").checked).toBe(true);
